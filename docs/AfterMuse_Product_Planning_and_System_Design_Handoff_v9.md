...
한국 도서 MVP의 Metadata Provider는 **Kakao 도서 검색 API**로 결정했다. 기존에 주요
후보로 논의하고 IMP-021에서 구현한 알라딘 OpenAPI는 신규 key 발급과 서비스 종료가
공지되어 기본 Provider에서 제외한다.
...
- Provider 이용 약관, 호출 한도와 상업적 이용 조건은 운영 전 다시 확인한다.
...
- Kakao 도서 검색 API의 운영 호출 한도와 상업적 이용 조건
...
5. 신규 책 준비 정책
   ...
   - 책 선택 시 기본 Metadata Provider의 서지정보를 우선 확보한다.
...
Metadata Provider를 통한 서지정보 확보는 도서 검색/선택 단계에서 수행한다. Core MVP의
기본 Provider는 Kakao 도서 검색 API다.
...
Day는 엄격한 마감일이 아니라, 매일 무엇을 구현할지 보기 쉽게 하는 작업 묶음이다. 알라딘
OpenAPI 종료 대응으로 IMP-025가 Critical Path에 추가되어 총 11개 작업 묶음으로
재기준화했으며, 실제 완료 여부는 각 IMP 체크박스로 관리한다.
