...
- [x] **IMP-085 — Core Home / 사용자 상태 연결**
  ...
  - **검증:** 실제 사용자 Reading(읽고 싶음, 읽는 중, 완독) 및 진행 중 Interview의 상태별 카드·빈 상태·직접 이동(읽기 계속, 인터뷰 시작, 이어하기)과 N+1 방지(5쿼리 고정)를 단위/통합 테스트와 `scripts/verify.py`로 검증했다.
...
- [x] **IMP-086 — Core Interview 재진입 UI**
  ...
  - **검증:** 첫 질문 전, 미답변 턴, 답변 저장 후, 진행 대기, 오류 복구 분기에서 GET 재진입 시 인터뷰·턴 수 및 답변 불변과 현재 단계 보존을 회귀 테스트와 `scripts/verify.py`로 검증했다.
