...
- 추가된 지침:
  - 실제 브라우저 검증의 명시적 실행 조건
  - 선택형 인간 UX 검토와 구현 완료 조건의 분리
  - 근거 없는 viewport 시나리오 반복 및 성공률 기준 금지
- 제거된 섹션: 없음
- 후속 작업: 기존 feature artifact는 소급 수정하지 않음
-->
...
주요 화면은 Normal, Empty, Loading, Error 상태와 Desktop/Mobile 레이아웃을 지원해야
하며, 네트워크 또는 LLM 오류로 사용자의 답변을 유실해서는 안 된다. Keyboard
navigation, 충분한 색 대비, 명확한 focus 상태 및 색상 외 상태 표현을 보장해야 한다.

Desktop/Mobile 지원은 제품 동작과 구현의 인수 조건이지만, 그 사실만으로 각 기능
작업에서 실제 브라우저 실행을 필수 검증으로 요구하지 않는다. 기본 검증은 Django test
client를 사용한 HTML·응답 계약, 접근성 속성, 상태 전이와 오류 처리 테스트 및 적용
가능한 정적 검사처럼 결정적이고 반복 가능한 기계적 증거를 우선한다.
...
4. 테스트는 내부 구현을 과도하게 mocking하지 않고 실제 비즈니스 상태와 전이를
   ...
   재현하는 회귀 테스트가 필요하다.
5. 실제 브라우저 검증은 사용자가 현재 작업에서 명시적으로 요청했거나, 승인된
   milestone·release 인수 계획이 해당 검증을 완료 조건으로 지정한 경우에만 수행한다.
   이 조건이 없으면 Codex 내부 브라우저, CUA, Playwright, Selenium 또는 이에 준하는
   브라우저 조작을 spec, plan, quickstart, tasks 또는 Convergence의 필수 작업으로
   추가하거나 실행해서는 안 된다. 사람이 수행할 선택형 UX 확인은 구현 완료 조건과
   분리해 기록하며 미구현 또는 부분 구현으로 판정하지 않는다. 브라우저 검증이 승인된
   경우에도 요구사항이 별도 반복 횟수를 지정하지 않았다면 대표 Desktop과 Mobile
   viewport에서 각 시나리오를 한 번만 확인하며, 실패 재현이나 flaky 동작 검증 근거 없이
   반복 횟수 또는 성공률 기준을 임의로 추가해서는 안 된다.
6. 완료 전 `uv run python scripts/verify.py`를 실행해야 한다. 전체 검증이 환경 제약으로
   불가능하면 실행한 개별 검사, 실패 원인 및 잔여 위험을 변경 보고에 기록해야 한다.
7. 제품 정책, 공개 계약, 기술 스택 또는 운영 절차가 바뀌면 같은 변경에서 README,
   관련 `docs/`, CHANGELOG 및 코드 내 계약 문서를 수술적으로 동기화해야 한다.
8. 헌법 원칙을 벗어나는 복잡성이나 예외는 plan의 Complexity Tracking에 필요성,
...
판단할 수 없는 AI 품질과 보안 경계는 인수 기준이나 별도 검토 증거를 남겨야 한다.
UX의 실제 브라우저 검토는 개발 워크플로 및 품질 게이트에서 정한 명시적 실행 조건이
성립한 경우에만 완료 증거로 요구한다. 위반을 발견하면 기능 확장보다 수정 또는
명시적인 헌법 개정을 우선한다.

**버전**: 1.1.0 | **비준일**: 2026-09-02 | **최근 개정일**: 2026-09-09
