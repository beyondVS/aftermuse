...
4. 같은 ISBN13 저장이 순차 또는 동시에 시도되어도 최종 Book은 한 건만 남아야 한다.
5. 중복 저장 실패는 기존 Book의 서지정보를 변경하지 않아야 한다.
6. `title`은 모델 검증에서 필수이며 빈 문자열을 허용하지 않는다.
7. 선택 문자열이 누락되면 빈 문자열을 유지하고 임의 값을 채우지 않는다.
8. 출간일이 누락되면 `NULL`을 유지하고 임의 날짜를 만들지 않는다.
...
- ISBN 유일성은 모델 검증 결과와 별개로 PostgreSQL 제약이 최종 보장한다. 경쟁 상황의
  패배 요청은 무결성 오류로 종료되어야 한다.
...
- ISBN13 유일성과 Django가 생성하는 LIKE 조회용 보조 index는 새 빈 테이블에 만든다.
- 생성 SQL에서 PK, 각 필드의 nullability, ISBN13 유일성과 보조 index를 확인한다.
