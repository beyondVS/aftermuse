...
기록이나 링크는 철저히 격리됩니다.
Day 10에서는 Interview 완료 상태에서 사용자가 확정한 답변들을 바탕으로 최초 독서노트
초안(Reflection Draft)을 보존·조회하고, LLM을 통해 답변 원문 근거 기반의 구조화 초안을
생성하며, 사용자의 수정본을 분리 보존하는 핵심 도메인 모델과 서비스 계약이 추가되었습니다(IMP-090·IMP-091).
`Reflection` 모델은 Interview와 1:1로 결합하며, 모델과 DB CHECK 제약 조건으로 최초 초안
본문(`draft_markdown`) 및 구조 데이터(`draft_sections`)의 사후 변조를 차단합니다. 사용자가
작성하는 수정본(`revised_markdown`)은 별도로 관리되며 원본 초안을 덮어쓰지 않습니다.
초안 생성(`generate_reflection_draft`)은 DB 트랜잭션 외부에서 수행되는 비영속 작업으로,
신뢰된 한국어 지침과 비신뢰 답변 턴 payload를 물리적으로 분리합니다. 네 Provider
(Fake, OpenAI, Gemini, Ollama) 모두 엄격한 Wire JSON 스키마를 따르며, 서버는 모든 문단이
실제 확정 답변의 정확한 연속 부분 문자열(`exact substring`)만을 인용하는지, 2자 이상 단어
토큰으로 연결되는지, 금지 형식(HTML, 링크, 이미지, 헤딩, 코드블록) 및 프롬프트 주입 지시
패턴이 없는지 엄격히 재검증합니다. Provider 장애 시에는 전용 `ReflectionGenerationError`
(Timeout, Unavailable, Rejected, ConfigurationError)로 격리되어 기존 인터뷰 상태나 레코드를
변경하지 않으며, API 키 등 민감정보를 에러에 노출하지 않습니다.
웹 UI 화면(생성/재시도 버튼, 수정 폼 템플릿, 완료 뷰) 및 실제 유료 LLM의 의미적 충실성 평가는
후속 작업(Day 11/12)의 범위이며, 기본 자동 검사는 외부 네트워크 격리 하에 수행됩니다.
