...
- **Primary Behavior 우선**: 주변 방어(hardening)보다 다음 순서를 우선하십시오.
  1. 제품 의도 (Product Intent)
  2. 핵심 사용자 동작 (Primary User Behavior)
  3. 필수 정확성 (Essential Correctness)
  4. 실제 불변조건 (Real Invariants)
  5. 필요한 만큼의 방어 (Hardening)
  방어 로직만 정교하고 핵심 사용자 동작이 미완성인 상태는 완료로 취급하지 마십시오.
- **현재 요구에 비례한 방어**:
  검증(validation), 가드, 동시성 제어, fallback, 재시도(retry), 복구 로직은 명시적 현재 요구사항, 실제 인증·인가·소유권·신뢰 경계, 영속 데이터 무결성, 트랜잭션 필수 불변조건, 비가역적·파괴적 작업, 이미 관찰된 실패 모드, 외부 시스템의 명시적 계약과 직접 연결될 때 우선합니다.
  이론적인 race condition, 미래 scale/배포 가정, 필요성이 미확인된 retry/idempotency 프로토콜, 모든 비정상 상태 조합, 현재 사용자 흐름과 무관한 세분화된 오류 계약 등 가상 위험 가능성만으로 요구사항을 새로 만들지 마십시오.
...
- **Mechanical Proxy 경계**: 수치화나 테스트가 쉽다는 이유만으로 lexical/token overlap, 문장부호 수, 블랙리스트, 고정 단락 수, 임의 길이 임계값, 특정 토큰 유무 같은 휴리스틱을 실제 제품 요구사항의 대체물로 삼지 마십시오. 이러한 proxy가 정상적인 유효 동작을 거부(hard reject)하거나 제품 의도보다 강한 제약이 된다면 완화하거나 제거하고 관찰 가능한 실제 동작과 불변조건을 검증하십시오.
- **언더엔지니어링 경계 (단순화의 한계)**: 단순성, 최소 diff, YAGNI 또는 코드량 감소를 이유로 현재 요구에 필요한 핵심 동작, 기존 코드가 제공하던 확인 가능한 동작 계약, 실제 도메인·데이터 불변조건, 프로젝트의 기존 검증·오류 처리 관례 중 현재 동작에 필요한 부분, 요구된 영속화나 부작용, 현재 정확성을 확인하는 데 필요한 테스트나 타입 정보를 축소·생략하지 마십시오. 미래의 가상 위험을 새로 방어하는 것과 현재 필요한 정확성을 최소화 명목으로 잘라내는 것은 엄격히 구분합니다.
...
- **요구사항 승격 게이트 (Requirement Promotion Gate)**:
  작업 중 새로운 edge case, failure mode, hardening 아이디어를 발견해도 현재 요구사항·인수 조건·태스크·테스트로 자동 승격하지 않습니다.
  현재 구현 범위로 승격하려면 실제 보안·신뢰 경계, 데이터 무결성, 비가역적 손실 위험, 명시적 현재 요구사항, 현재 핵심 사용자 흐름, 이미 관찰된 실패, 명확한 외부 계약 중 하나와 직접 연결되어야 합니다.
  그 외 항목은 필요 시 optional/future concern으로 기록할 수 있으나 현재 구현 계약에 포함하지 않으며, 별도의 위험 점수나 의무 체크리스트를 강제하지 않습니다.
...
### 결과 반영과 검토 종료 (Review Closure)

검토 결과가 수정이나 진행 중단을 요구하면 해당 내용을 반영한 뒤 필요한 경우 다시 검토합니다.

- **재검토 범위의 수렴 (Verification Focus)**: 첫 검토에서 현재 범위의 중요한 문제를 충실히 도출한 후, 지적 사항 수정에 따른 재검토는 검증(verification)으로 취급합니다. 재검토에서는 기존 지적 사항의 해결 여부와 현재 범위에서 새로 확인된 명백한 차단형 결함(blocking defect)만 확인합니다.
- **무한 루프 방지**: 새로운 선택적 개선안(optional improvement), 추측성 방어(speculative hardening), 미래 추상화를 재검토 과정에서 현재 구현 범위로 계속 추가하지 않습니다.
- **종료 조건**: 핵심 사용자 동작, 필수 정확성 및 실제 불변조건이 충족되고 차단형 결함이 없으면 검토를 종료합니다.
- **실행 불가 보고**: 이 섹션에 따라 필요한 독립 검토 수단을 사용할 수 없거나 실행에 실패한 경우, 검토한 것으로 간주하지 않고 그 사실과 독립 검토 부재에 따른 잔여 위험을 최종 보고에 명시합니다.
